...
from thirdparty.grpc.tests import (
    logging_pool,
    test_common,
    test_constants,
    test_control,
...
def _async_unary_unary_multi_callable(channel):
    return channel.async_unary_unary(_UNARY_UNARY)
...
class RPCTest(unittest.TestCase):
    ...
    def testSuccessfulUnaryRequestBlockingUnaryResponse(self):
        ...
        response = multi_callable(
            request, metadata=(("test", "SuccessfulUnaryRequestBlockingUnaryResponse"),)
        )
    ...
    def testSuccessfulUnaryRequestFutureUnaryResponse(self):
        ...
        response_future = multi_callable.future(
            request, metadata=(("test", "SuccessfulUnaryRequestFutureUnaryResponse"),)
        )
        response = response_future.result()

        self.assertIsNotNone(response_future)
        self.assertEqual(expected_response, response)
        self.assertIsNone(response_future.exception())
        self.assertIsNone(response_future.traceback())

    @unittest.skip("asyncio not implemented")
    @unittest.skipIf(six.PY2, "asyncio not supported in Python 2")
    def testSuccessfulAsyncUnaryRequestUnaryResponse(self):
        request = b"\x07\x08"
        expected_response = self._handler.handle_unary_unary(request, None)

        multi_callable = _async_unary_unary_multi_callable(self._channel)
        response_future = multi_callable(
            request, metadata=(("test", "SuccessfulUnaryRequestFutureUnaryResponse"),)
        )
        response = response_future.result()
    ...
    def testSuccessfulUnaryRequestStreamResponse(self):
        ...
        response_iterator = multi_callable(
            request, metadata=(("test", "SuccessfulUnaryRequestStreamResponse"),)
        )
    ...
    def testSequentialInvocations(self):
        ...
        expected_first_response = self._handler.handle_unary_unary(first_request, None)
    ...
    def testConcurrentFutureInvocations(self):
        ...
        for index in range(test_constants.THREAD_CONCURRENCY):
            request_iterator = iter(requests)
            response_future = multi_callable.future(
                request_iterator, metadata=(("test", "ConcurrentFutureInvocations"),)
            )
            response_futures[index] = response_future
        responses = tuple(
            response_future.result() for response_future in response_futures
    ...
    def testCancelledUnaryRequestUnaryResponse(self):
        ...
        with self._control.pause():
            response_future = multi_callable.future(
                request, metadata=(("test", "CancelledUnaryRequestUnaryResponse"),)
            )
    ...
    def testFailedUnaryRequestFutureUnaryResponse(self):
        ...
        with self._control.fail():
            response_future = multi_callable.future(
                request, metadata=(("test", "FailedUnaryRequestFutureUnaryResponse"),)
            )
            response_future.add_done_callback(callback)
            value_passed_to_callback = callback.value()
    ...
    def testIgnoredUnaryRequestFutureUnaryResponse(self):
        ...
        multi_callable.future(
            request, metadata=(("test", "IgnoredUnaryRequestFutureUnaryResponse"),)
        )
    ...
    def _consume_one_stream_response_unary_request(self, multi_callable):
        ...
        response_iterator = multi_callable(
            request, metadata=(("test", "ConsumingOneStreamResponseUnaryRequest"),)
        )
    ...
    def _cancelled_unary_request_stream_response(self, multi_callable):
        ...
        with self._control.pause():
            response_iterator = multi_callable(
                request, metadata=(("test", "CancelledUnaryRequestStreamResponse"),)
            )
            self._control.block_until_paused()
    ...
    def _failed_unary_request_stream_response(self, multi_callable):
        ...
        with self.assertRaises(grpc.RpcError) as exception_context:
            with self._control.fail():
                response_iterator = multi_callable(
                    request, metadata=(("test", "FailedUnaryRequestStreamResponse"),)
                )
                next(response_iterator)
    ...
    def _ignored_stream_request_stream_response(self, multi_callable):
        ...
        multi_callable(
            request_iterator, metadata=(("test", "IgnoredStreamRequestStreamResponse"),)
        )
