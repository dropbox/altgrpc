...
setup(
    ...
    ext_modules=[
        Extension(
            ...
            sources=[{c_file}],
            include_dirs=['.'],
            #extra_compile_args=['-std=c++14', '-fsanitize=address'],
...
def dbx_pyx_library(
    ...
    shared_object_name = module_name.replace(".", "/")
    contents = {
        "cpython-27": py_srcs + [shared_object_name + ".so"],
        "cpython-38": py_srcs + [shared_object_name + ".cpython-38-x86_64-linux-gnu.so"],
    }
    dbx_py_local_piplib(
        name = name,
